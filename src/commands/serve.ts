  ...
  program
    ...
    .action(async () => {
      ...
      if (!config.loggedIn || !config.authToken) {
        console.log('');
        console.log(ERROR('  ❌ Not logged in. Active Bob requires Tier 3.'));
      ...
      if (!conversationId) {
        ...
        console.log(MUTED('  Active Bob must be bound to a conversation.'));
        console.log(MUTED('  Run `bob conversations join` first, then `bob serve`.'));
        console.log('');
        return;
      }
      ...
      if (!config.localEndpoint) {
        ...
        console.log(MUTED('  Active Bob uses your local model for sovereign execution.'));
      ...
      if (BLOCKED_TIERS.includes(userTier)) {
        ...
        console.log('');
        console.log(topRule());
        console.log(row(ERROR('❌  Active Bob requires a paid subscription.')));
        console.log(row(MUTED(`▸ Your current tier: ${userTier}`)));
        console.log(hRule());

        if (isOrgUser) {
          console.log(row(BRAND_SECONDARY('🏢  Contact your organization administrator to upgrade.')));
          console.log(row(MUTED(`▸ Organization: ${domain}`)));
          console.log(row(MUTED('▸ Admin Dashboard: https://bobs-workshop.web.app/#/bobsadmindashboard')));
        } else {
          console.log(row(BRAND_SECONDARY('🚀  Upgrade to unlock remote execution:')));
          console.log(emptyRow());
          console.log(row(MUTED('▸ Starter  — 15s polling, 15 min idle timeout')));
          console.log(row(MUTED('▸ Pro      — 10s polling, sleep mode, 1 hour idle timeout')));
          console.log(row(MUTED('▸ Power    — 2s polling, sleep mode, never disconnects')));
          console.log(emptyRow());
          console.log(row(INFO('▸ Upgrade at: https://bobs-workshop.web.app/#/pricing')));
        }
...
async function startActiveBob(
  ...
  console.log('');
  console.log(BORDER('  ╔══════════════════════════════════════════════════════════╗'));
  console.log(BORDER('  ║') + INFO('  🌐 Bob Serve — Active Bob Online                       ') + BORDER('║'));
  console.log(BORDER('  ╠══════════════════════════════════════════════════════════╣'));
  console.log(BORDER('  ║') + MUTED(`  Account:  ${config.email}`));
  console.log(BORDER('  ║') + MUTED(`  Machine:  ${machineId}`));
  console.log(BORDER('  ║') + MUTED(`  Project:  ${projectName} (${process.cwd()})`));
  console.log(BORDER('  ║') + MUTED(`  Session:  ${sessionId.slice(0, 30)}...`));
  console.log(BORDER('  ║') + MUTED(`  Convo:    ${conversationId?.slice(0, 24)}...`));
  console.log(BORDER('  ║') + BRAND_SECONDARY(`  Tier:     ${userTier}`));
  console.log(BORDER('  ║') + MUTED(`  Polling:  every ${tierConfig.activeInterval / 1000}s`));
  if (tierConfig.sleepInterval) {
    console.log(BORDER('  ║') + MUTED(`  Sleep:    every ${tierConfig.sleepInterval / 1000}s after ${tierConfig.idleThreshold! / 60000} min idle`));
  } else {
    console.log(BORDER('  ║') + MUTED('  Sleep:    disabled'));
  }
  if (tierConfig.extendedIdleTimeout) {
    console.log(BORDER('  ║') + MUTED(`  Auto-exit: after ${tierConfig.extendedIdleTimeout / 60000} min idle`));
  } else {
    console.log(BORDER('  ║') + SUCCESS('  Auto-exit: never (Power tier)'));
  }
  console.log(BORDER('  ║'));
  console.log(BORDER('  ║') + MUTED('  Capabilities: chat, consult, push, index, analyse, backup, restore'));
  console.log(BORDER('  ║') + MUTED('  Press Ctrl+C to stop.'));
  console.log(BORDER('  ╚══════════════════════════════════════════════════════════╝'));
  console.log('');
  ...
  try {
    ...
    console.log(SUCCESS('  ✅ Active Bob registered. Listening for commands...'));
    console.log('');
  } catch (error: any) {
    console.log(ERROR(`  ❌ Failed to register: ${error.message}`));
    return;
  }
  ...
  const cleanup = async () => {
    ...
    running = false;
    console.log('');
    console.log(MUTED('  🔌 Shutting down Active Bob...'));
    try {
      await callCloudFunction('deregisterRemoteDaemonSession', { conversationId, sessionId });
      console.log(MUTED('  ✅ Session deregistered. Bob is offline.'));
    } catch {
      console.log(MUTED('  ⚠️  Could not deregister (will timeout automatically).'));
    }
    console.log('');
    process.exit(0);
  };
  ...
  while (running) {
    ...
    if (tierConfig.extendedIdleTimeout && timeSinceLastCommand > tierConfig.extendedIdleTimeout) {
      console.log('');
      console.log(BRAND_SECONDARY(`  ⏸️  No commands received in ${Math.round(tierConfig.extendedIdleTimeout / 60000)} minutes.`));
      console.log(MUTED('  Active Bob is going offline. Run `bob serve` to restart.'));
      console.log('');
      try {
        await callCloudFunction('deregisterRemoteDaemonSession', { conversationId, sessionId });
      } catch { }
    ...
    if (tierConfig.sleepInterval && tierConfig.idleThreshold) {
      ...
      if (shouldSleep && !isSleeping) {
        ...
        console.log(MUTED(`  [${timestamp}] 💤 Entering sleep mode (polling every ${tierConfig.sleepInterval / 1000}s)...`));
      }
    }
    ...
    // ─── POLL ───
    try {
      const result = await callCloudFunction('pollRemoteCommands', { conversationId, sessionId });
      ...
      if (result?.command) {
        ...
        if (isSleeping) {
          isSleeping = false;
          console.log(SUCCESS('  ⚡ Waking up — command received!'));
        }

        lastCommandTime    = Date.now();
        const timestamp    = new Date().toLocaleTimeString();
        console.log(BRAND_SECONDARY(`  [${timestamp}] ⏳ Received: ${type}${payload.message ? ` "${payload.message.slice(0, 40)}${payload.message.length > 40 ? '...' : ''}"` : ''}`));
        ...
        if (commandResult.success) {
          console.log(SUCCESS(`  [${endTimestamp}] ✅ Completed: ${type}`));
        } else {
          console.log(ERROR(`  [${endTimestamp}] ❌ Failed: ${type} — ${commandResult.error || 'unknown'}`));
        }
        console.log('');
      }
      ...
      if (error.message?.includes('Session expired')) {
        console.log(ERROR('  ❌ Session expired. Run `bob login` and restart `bob serve`.'));
        running = false;
      }
    }
