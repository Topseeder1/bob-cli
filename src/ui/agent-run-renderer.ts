...
// ─── LEAKED CONTENT DETECTION ────────────────────────────────────
// Filters lines that are leaked tool content or code from display
...
export function renderMissionHeader(
  ...
  console.log(
    BORDER('  ║') +
    `  ${WHITE(mission.description.slice(0, 56))}` +
    (mission.description.length > 56 ? GRAY('...') : '')
  );
  console.log(
    BORDER('  ║') +
    GRAY(`  Mission: ${mission.id}  │  Tasks: ${mission.tasks.length}  │  Team: ${agentNames.length} agents`)
  );
  console.log(BORDER('  ║'));
...
export function renderTaskMap(
  ...
  console.log(
    ...
    GRAY('░'.repeat(empty)) +
    GRAY('] ') +
    barColor(`${summary.percentComplete}%`) +
    GRAY(` — ${summary.completed}/${summary.total}`)
  );
...
function renderTaskRow(
  ...
  const instruction = task.instruction.slice(0, 44) +
    (task.instruction.length > 44 ? '...' : '');
  ...
  if (
    (task.status === 'stagnated' || task.stagnationCount > 0) &&
    task.notes.length > 0
  ) {
    const lastNote = task.notes[task.notes.length - 1];
    console.log(
      DIRECTOR_COLOR('  ║') +
      ORANGE(`    ⚠️  ${lastNote.slice(0, 60)}${lastNote.length > 60 ? '...' : ''}`)
    );
  }
}
...
export function renderExecutionEvent(
  ...
  switch (event.type) {
    ...
    case 'thinking': {
      if (event.agentName === 'directorBob') {
        console.log(
          `${timeStamp} ${DIRECTOR_COLOR('🎬 DirectorBob:')} ${GRAY(event.message)}`
        );
      } else {
    ...
    case 'response': {
      if (event.agentName === 'directorBob') {
        ...
        for (const line of lines) {
          console.log(
            DIRECTOR_COLOR('  │') +
            DIRECTOR_COLOR(`  ${line.slice(0, 58)}`)
          );
        }
        console.log(DIRECTOR_COLOR('  └────────────────────────────────────────────────────────┘'));
        console.log('');
...
export function renderMissionComplete(mission: AgentMission): void {
  ...
  const duration = mission.startedAt && mission.completedAt
    ? Math.round(
        (new Date(mission.completedAt).getTime() -
         new Date(mission.startedAt).getTime()) / 1000
      )
    : 0;
  ...
  if (summary.failed > 0) {
    console.log(BORDER('  ║') + RED(`  ❌ Tasks failed:      ${summary.failed}`));
  }
  if (summary.stagnated > 0) {
    console.log(BORDER('  ║') + ORANGE(`  ⚠️  Tasks stagnated:  ${summary.stagnated}`));
  }
  if (summary.skipped > 0) {
    console.log(BORDER('  ║') + GRAY(`  ⏭️  Tasks skipped:    ${summary.skipped}`));
  }
  console.log(BORDER('  ║'));
  ...
  if (duration > 0) {
    console.log(BORDER('  ║') + GRAY(`  ⏱  Duration:         ${durationLabel}`));
  }
  console.log(BORDER('  ╠══════════════════════════════════════════════════════════╣'));
...
export function renderPausedBanner(reason?: string): void {
  ...
  if (reason) {
    console.log(AMBER('  ║') + RED(`  ${reason.slice(0, 56)}`));
  }
  console.log(AMBER('  ║'));
  console.log(AMBER('  ║') + GRAY('  Options:'));
  console.log(AMBER('  ║') + CYAN('    /resume') + GRAY('                       — continue'));
  console.log(AMBER('  ║') + CYAN('    /inject "directive"') + GRAY('            — guide the agent'));
  console.log(AMBER('  ║') + CYAN('    /set-target <agent> <n>') + GRAY('        — lower the target'));
  console.log(AMBER('  ║') + CYAN('    /skip <taskId>') + GRAY('                 — skip stuck task'));
  console.log(AMBER('  ║') + CYAN('    bob agent chat <name>') + GRAY('          — talk to agent'));
  console.log(AMBER('  ║') + CYAN('    /abort') + GRAY('                         — stop everything'));
  console.log(AMBER('  ╚══════════════════════════════════════════════════════════╝'));
  console.log('');
}
...
export function renderDirectorPlanning(): void {
  ...
  console.log(DIRECTOR_COLOR('  ║'));
  console.log(DIRECTOR_COLOR('  ║') + GRAY('  ◎ Reading agent specialties and personas'));
  console.log(DIRECTOR_COLOR('  ║') + GRAY('  ◎ Scanning project structure and existing files'));
...
export function handleRunCommand(
  ...
  if (trimmed === '/view-targets') {
    ...
    for (const task of mission.tasks) {
      ...
      lines.push(
        ...
        CYAN(`${task.satisfactionTarget}%`) +
        (state.satisfactionOverrides[task.assignedTo] !== undefined
          ? GREEN(' (user override)')
          : '') +
        (task.status === 'running' ? CYAN(' ← active') : '')
      );
    }
  ...
  if (setTargetMatch) {
    ...
    return {
      handled: true,
      message: GREEN(`  ✅ @${agentName} satisfaction target → ${target}%`),
    };
  }
  ...
  if (injectMatch) {
    ...
    return {
      handled: true,
      message: GREEN(`  ✅ Director note queued: "${injectMatch[1].slice(0, 50)}"`),
    };
  }
  ...
  if (skipMatch) {
    ...
    if (task) {
      ...
      return {
        handled: true,
        message: GRAY(`  ⏭️  Task skipped: ${task.instruction.slice(0, 50)}`),
      };
    }
