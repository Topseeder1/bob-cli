...
async function runAutoFix(options) {
  ...
  console.log(GRAY("     /skip <file> to skip a task. /done to stop early."));
  console.log(GRAY("  \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"));
  console.log("");
...
APPROVED (will auto-fix): ${approved.map((r) => `${r.suggestion.filePath}: ${r.suggestion.title || r.suggestion.description} (${r.confidence}%)`).join("\n")}

NEEDS REVIEW: ${review.map((r) => `${r.suggestion.filePath}: ${r.suggestion.title || r.suggestion.description} (${r.confidence}%)`).join("\n")}

DISMISSED: ${dismissed.map((r) => `${r.suggestion.filePath}: ${r.suggestion.title || r.suggestion.description} (${r.confidence}%)`).join("\n")}

Help the user understand and decide. Be concise.`;
  const history = [
    ...
    const prompt = () => {
      rl.question(chalk.green("  You: "), async (input) => {
        ...
        if (trimmed.toLowerCase().startsWith("skip ") || trimmed.toLowerCase().startsWith("remove ")) {
          ...
          console.log(removed > 0 ? chalk.yellow(`  \u23F8\uFE0F  Removed ${removed} item(s) matching "${target}"`) : GRAY(`  No items found matching "${target}"`));
          console.log("");
          prompt();
          return;
        }
...
function detectCategory(suggestion) {
  ...
  for (const cat of ["bugs", "features", "improvements", "upgrades"]) {
    ...
    for (const item of items) {
      if (item.title === suggestion.title && item.description === suggestion.description) {
        return cat;
      }
    }
  }
